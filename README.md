...
一個安靜住在選單列的 macOS 語音轉文字 App — 按住快捷鍵、說話、放開，潤飾好的文字就自動貼上了。
...
- **簡單，但夠用**：沒有複雜的模式，只有單純的語音輸入與轉錄，只要按住快捷鍵、說話、放開，潤飾過的文字會自動貼到游標位置
- **為台灣用戶打造**：指定轉錄語音與文字都是繁體中文，不用擔心不小心跑出簡體中文。
- **重視隱私，開箱即用**：不要求多餘權限、不需要註冊帳號、你的 API key 存在自己電腦，所有轉錄都不會傳送給 Purri，只會給你指定的轉錄服務提供廠商。
- **轉錄多少付多少**： Purri 免費下載使用，自己用自己的 API key，轉錄多少就付多少，不用被訂閱制綁住。
...
- **長按模式** — 按住快捷鍵說話，放開即停止（Push-to-Talk）
- **短按模式** — 按一下快捷鍵開始，再按一下就停止，也可以在靜音幾秒後自動停止。


### AI 文字潤飾

零散口語潤飾後可以直接送出，還可以重新潤飾成其他版本
